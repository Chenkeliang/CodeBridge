...
<body>
  ...
  <script>
    ...
    $('work-form').addEventListener('submit', async (event) => { event.preventDefault(); $('error').textContent = ''; try { const scope = $('scope').value.split(',').map((value) => value.trim()).filter(Boolean); const item = await api('/v1/work-items', { method:'POST', body: JSON.stringify({ conversation_id: 'web:' + crypto.randomUUID(), title: $('work-title').value, agent_id: $('agent').value, workflow_id: $('workflow').value || null, mode: $('mode').value, workspace_scope: scope, message: $('message').value }) }); await selectItem(item.id); await startRun(); } catch (error) { $('error').textContent = error.message; } });
