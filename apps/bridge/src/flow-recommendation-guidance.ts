...
export function buildFlowRecommendationGuidance(flows: FlowRecord[]): string {
  ...
  return [
    ...
    "每个非默认输入都必须有同名 evidence；用户消息或附件中明确出现的值用 source=agent_extracted、inferred=false，evidence_ref 指向 source_refs 中对应的具体行或片段。",
    'evidence 精确格式示例：{"evidence":{"oid":{"source":"agent_extracted","evidence_ref":"message#line:1","inferred":false}}}；多个字段必须逐字段填写，不能只写 source_refs。',
