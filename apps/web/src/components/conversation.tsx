...
export function ProjectionItem({ approvals, cwd, item, onApproval }: { approvals: ApprovalRecord[]; cwd: string | null; item: ConversationProjection; onApproval: (item: ApprovalProjection, approve: boolean) => Promise<void> }) {
  if (item.kind === "user") return <article className="grid min-w-0 justify-items-end gap-2"><span className={cn("font-brand text-xs font-normal uppercase tracking-[0.1em]", "text-muted")}>你</span><div className={cn("max-w-[72%] min-w-0 [overflow-wrap:anywhere] rounded-xl px-3.5 py-3 text-sm leading-6", "text-ink", "bg-accent-soft")}>{item.content}</div></article>;
