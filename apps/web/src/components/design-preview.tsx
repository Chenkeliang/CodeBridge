    ...
    <div className={cn("grid h-[100dvh] min-h-[100dvh] min-w-[1040px] grid-cols-[60px_286px_minmax(0,1fr)] overflow-hidden font-sans text-[13px] tracking-[-0.01em]", "bg-canvas text-ink")} data-theme={theme}>
      ...
      <aside className={cn("flex min-h-0 min-w-0 flex-col border-r", "bg-sidebar", "border-line")}>
        ...
        {area === "agents" ? <>
          ...
          <div className="min-h-0 flex-1 overflow-auto px-2.5 pb-4">
            <div className={cn("px-2.5 py-2 text-[10px] font-semibold uppercase tracking-[0.1em]", "text-faint")}><span>Sessions</span><span className="float-right font-mono">{filteredSessions.length}</span></div>
            {filteredSessions.map((session, index) => <button className={cn("relative grid w-full gap-1 rounded-md border border-transparent px-3 py-2.5 text-left transition-colors hover:opacity-80", "text-ink", activeSession === session && cn("bg-surface", "border-line", "shadow-card"))} key={session} onClick={() => { setActiveSession(session); notify(`已切换到 ${session}`); }} type="button"><span className="truncate pr-3 text-xs font-semibold">{session}</span><span className={cn("flex items-center gap-1.5 text-[10px]", "text-muted")}>{index === 0 && <Pin className={cn("size-3", "text-warning")} />}<span>{index === 0 ? "Running" : "Ready"}</span><span>·</span><span className="font-mono">{index === 0 ? "now" : `${index}h`}</span></span>{activeSession === session && <span className={cn("absolute right-2.5 top-3.5 size-1.5 rounded-full", "bg-accent")} />}</button>)}
            {!filteredSessions.length && <div className={cn("px-3 py-8 text-center text-xs", "text-muted")}>没有匹配的 Session</div>}
          </div>
          <footer className={cn("flex items-center justify-between border-t px-4 py-3 text-[10px]", "border-line", "text-muted")}><Button className={cn("h-auto p-0 text-[10px] hover:opacity-80", "text-muted")} onClick={() => notify("已归档 Session")} size="sm" variant="ghost">已归档</Button><span className="font-mono">{sessionSets[activeAgent]?.length ?? 0} active</span></footer>
        </> : <div className="min-h-0 flex-1 overflow-auto px-2.5 pb-4"><div className={cn("px-2.5 py-2 text-[10px] font-semibold uppercase tracking-[0.1em]", "text-faint")}>Published</div>{flowNames.map((name, index) => <button className={cn("flex w-full items-center gap-2 rounded-md border border-transparent px-3 py-2.5 text-left text-xs transition-colors hover:opacity-80", "text-ink")} key={name} onClick={() => notify(`Flow · ${name}`)} type="button"><Workflow className={cn("size-3.5", "text-muted")} /><span className="min-w-0 flex-1 truncate">{name}</span><span className={cn("font-mono text-[10px]", "text-faint")}>v1.{index + 1}</span></button>)}</div>}
      </aside>
      ...
      <main className={cn("flex min-h-0 min-w-0 flex-col overflow-hidden", "bg-canvas text-ink")}>
        <header className={cn("flex min-h-[72px] items-center justify-between gap-5 border-b px-8 py-4", "border-line")}>
          ...
          <div className={cn("flex items-center gap-2 text-[11px]", "text-muted")}><span className={cn("size-1.5 rounded-full", "bg-accent")} /><span>Running</span><code className={cn("rounded border px-1.5 py-0.5 font-mono text-[10px]", "bg-surface-soft", "text-ink-soft", "border-line")}>run_7a31</code><Button aria-label="更多操作" className={cn("size-8 px-0 hover:opacity-80", "text-muted")} onClick={() => notify("Session actions")} size="icon" variant="ghost"><MoreHorizontal className="size-4" /></Button></div>
        </header>
        ...
        <section className="min-h-0 flex-1 overflow-auto px-8 pt-7" aria-label="Session conversation"><div className="mx-auto w-full max-w-[880px] pb-6">
          <div className={cn("mb-7 flex items-center justify-between gap-3 border-b py-2 text-[11px]", "border-line", "text-muted")}><div className="flex items-center gap-2"><span className={cn("inline-flex items-center gap-1.5 rounded px-2 py-1 text-[10px] font-semibold", "bg-accent-soft", "text-ink-soft")}><Circle className="size-1.5 fill-current" />Run active</span><span>Agent is working through the current plan</span></div><code className={cn("font-mono text-[10px]", "text-faint")}>workspace / CodeBridge</code></div>
        ...
        <footer className="px-8 pb-5 pt-3"><div className={cn("mx-auto w-full max-w-[880px] rounded-xl border", "bg-surface", "border-line-strong", "shadow-panel")}><div className="flex items-center gap-1.5 px-3 pt-2.5"><ContextChip label="Auto" /><ContextChip label="Agent default" /><ContextChip label="Workspace" /><ContextChip label="Flow · Automatic" /><span className="flex-1" /><span className={cn("hidden text-[10px] sm:inline", "text-faint")}>⌘ ↵ to send</span></div><Textarea aria-label="输入目标" className={cn("min-h-[70px] resize-none border-0 bg-transparent px-3.5 py-3 text-sm shadow-none focus:border-0 focus:ring-0", "text-ink")} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && (event.metaKey || event.ctrlKey)) { event.preventDefault(); sendMessage(); } }} placeholder="输入目标，或继续调整当前工作…" value={draft} /><div className="flex items-center justify-between gap-3 px-3 pb-2.5"><div className="flex items-center gap-1"><Button aria-label="添加文件" className={cn("size-7 px-0 hover:opacity-80", "text-muted")} onClick={() => notify("Attach files")} size="icon" variant="ghost"><Paperclip className="size-3.5" /></Button><Button aria-label="添加上下文" className={cn("size-7 px-0 text-xs hover:opacity-80", "text-muted")} onClick={() => notify("Add context")} size="icon" variant="ghost"><span>@</span></Button><Button aria-label="Agent commands" className={cn("size-7 px-0 text-xs hover:opacity-80", "text-muted")} onClick={() => notify("Agent commands")} size="icon" variant="ghost"><span>/</span></Button></div><Button aria-label="发送" className={cn("size-8 px-0 hover:opacity-80", "bg-accent", "text-accent-ink")} disabled={!draft.trim()} onClick={sendMessage} size="icon"><Send className="size-4" /></Button></div></div></footer>
      </main>
...
function ContextChip({ label }: { label: string }) {
  return <button className={cn("inline-flex min-h-6 items-center gap-1.5 rounded border px-2 text-[10px] transition-opacity hover:opacity-80", "bg-surface-soft", "text-muted", "border-line")} type="button"><span className={cn("font-semibold", "text-ink-soft")}>{label}</span></button>;
}
