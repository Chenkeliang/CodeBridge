...
export function SessionPanel({ agent, activeSessionCount, area, archivedSessionCount, flows, flowId, loading, query, sessions, selectedSessionId, showArchived, onCreate, onFlow, onQuery, onRefresh, onSession, onUpdateSession, onDeleteSession, onToggleArchived }: {
  ...
  return <aside className={cn("flex min-h-0 min-w-0 flex-col border-r", "bg-sidebar", "border-line")}>
    <header className="flex items-start justify-between gap-3 px-5 pb-4 pt-6">
      <div className="min-w-0"><p className={cn("mb-1 font-brand text-[11px] font-normal uppercase tracking-[0.1em]", "text-muted")}>{area === "agents" ? "当前 Agent" : "目录"}</p><h1 className={cn("truncate font-brand text-lg font-normal tracking-[-0.035em]", "text-ink")}>{area === "agents" ? agent?.display_name ?? "Agents" : "Flows"}</h1><p className={cn("mt-1.5 flex items-center gap-1.5 text-[11px]", "text-muted")}><Circle className={cn("size-1.5 fill-current", area === "agents" && agent?.status === "healthy" ? "text-success" : "text-faint")} />{area === "agents" ? (agent?.status === "healthy" ? `${sessions.length} 个会话` : `${statusLabel[agent?.status ?? "unavailable"] ?? agent?.status ?? "不可用"} · ${sessions.length} 个会话`) : `${flows.length} 个已发布定义`}</p></div>
...
export function SessionHeader({ agent, session, menuOpen, menuView, panelOpen, renameDraft, onDelete, onMenu, onMenuView, onRenameDraft, onTogglePanel, onUpdate }: {
  ...
  return <header className={cn("flex min-h-[72px] shrink-0 items-center justify-between gap-5 border-b px-8 py-4", "border-line")}>
    ...
    {session && <div className="relative flex items-center gap-2">
      {session.status !== "idle" && <span className={cn("hidden items-center gap-1.5 text-[11px] sm:flex", "text-muted")}><span className={cn("size-1.5 rounded-full", session.status === "active" ? "bg-success" : "bg-faint")} />{statusLabel[session.status] ?? session.status}</span>}
