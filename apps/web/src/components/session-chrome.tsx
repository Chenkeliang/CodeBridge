...
function SessionRow({ session, selected, onSession, onUpdateSession, onDeleteSession }: {
  ...
  return <div className="group relative [content-visibility:auto] [contain-intrinsic-size:60px]">
    <button className={cn("relative grid w-full gap-1 rounded-md border border-transparent px-3 py-2.5 pr-10 text-left transition-colors hover:opacity-80", "text-ink", selected && cn("bg-surface", "border-line", "shadow-card"))} onClick={() => { closeMenu(); onSession(session); }} title={title} type="button">
      ...
      <span className={cn("flex items-center gap-1.5 text-[11px]", "text-muted")}>{session.pinned_at && <Pin className={cn("size-3", "text-warning")} />}<span>{statusLabel[session.status] ?? session.status}</span><span>·</span><time className="font-mono">{relativeTime(session.updated_at)}</time></span>
...
export function SessionPanel({ agent, activeSessionCount, area, archivedSessionCount, flows, flowId, loading, query, sessions, selectedSessionId, showArchived, onCreate, onFlow, onQuery, onRefresh, onSession, onUpdateSession, onDeleteSession, onToggleArchived }: {
  ...
  return <aside className={cn("flex min-h-0 min-w-0 flex-col border-r", "bg-sidebar", "border-line")}>
    <header className="flex items-start justify-between gap-3 px-5 pb-4 pt-6">
      <div className="min-w-0"><p className={cn("mb-1 font-brand text-[11px] font-normal uppercase tracking-[0.1em]", "text-muted")}>{area === "agents" ? "当前 Agent" : "目录"}</p><h1 className={cn("truncate font-brand text-lg font-normal tracking-[-0.035em]", "text-ink")}>{area === "agents" ? agent?.display_name ?? "Agents" : "Flows"}</h1><p className={cn("mt-1.5 flex items-center gap-1.5 text-[11px]", "text-muted")}><Circle className={cn("size-1.5 fill-current", area === "agents" && agent?.status === "healthy" ? "text-success" : "text-faint")} />{area === "agents" ? `${statusLabel[agent?.status ?? "unavailable"] ?? agent?.status ?? "Unavailable"} · ${sessions.length} sessions` : `${flows.length} 个已发布定义`}</p></div>
    ...
    {area === "agents" ? <>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2.5 pb-4">
        <div className={cn("px-2.5 py-2 text-[11px] font-semibold uppercase tracking-[0.1em]", "text-faint")}><span>{showArchived ? "已归档" : "会话"}</span><span className="float-right font-mono">{showArchived ? archivedSessionCount : activeSessionCount}</span></div>
      ...
      <footer className={cn("border-t px-3 py-2", "border-line")}><button className={cn("flex h-8 w-full items-center gap-2 rounded-md px-2 text-left text-[11px] transition-opacity hover:opacity-80", "text-muted")} onClick={onToggleArchived} type="button"><Archive className="size-3.5" /><span className="flex-1">{showArchived ? "返回 Sessions" : "已归档"}</span><span className="font-mono text-[11px]">{showArchived ? activeSessionCount : archivedSessionCount}</span></button></footer>
    </> : <div className="min-h-0 flex-1 overflow-y-auto px-2.5 pb-4"><div className={cn("px-2.5 py-2 text-[11px] font-semibold uppercase tracking-[0.1em]", "text-faint")}>已发布</div>{flows.map((flow) => <button className={cn("flex w-full items-center gap-2 rounded-md border border-transparent px-3 py-2.5 text-left text-xs transition-colors hover:opacity-80", "text-ink", flowId === flow.flow_id && cn("bg-surface", "border-line"))} key={flow.flow_id} onClick={() => onFlow(flow.flow_id)} type="button"><Workflow className={cn("size-3.5", "text-muted")} /><span className="min-w-0 flex-1 truncate">{flow.name || flow.flow_id}</span><span className={cn("font-mono text-[11px]", "text-faint")}>{flow.kind}</span></button>)}{!flows.length && <div className={cn("px-3 py-8 text-center text-xs", "text-muted")}>暂无已发布 Flow</div>}</div>}
  </aside>;
}

export function SessionHeader({ agent, session, menuOpen, menuView, panelOpen, renameDraft, onDelete, onMenu, onMenuView, onRenameDraft, onTogglePanel, onUpdate }: {
  agent: AgentProfile | null;
