...
- `font-brand` 用于页面级标题（面板头、设置页、空态显示）；其字体栈内建 CJK 直通回退（Departure Mono 只覆盖拉丁字形，中文字形直接落系统黑体，不经等宽字体）。中文标题因此可以用 `font-brand` 保持层级一致，但像素感只体现在拉丁字符与数字上。
- `font-brand` 不用于长篇正文。
...
- 产品识别使用 AGNET 像素传送门标记（8×8 像素网格：深色门框 + 门缝透光，象征 Channel 与 Agent 之间的网关）；Agent 自身仍使用各 Agent 的官方几何图标，不能用像素化 AGNET 标记冒充 Agent Logo。
- 品牌资产位于 `apps/web/public/brand/`：`agnet-mark.svg`（Paper 图标）、`agnet-mark-carbon.svg`（Carbon 图标）、`agnet-logo.svg` / `agnet-logo-carbon.svg`（横向字标组合，字标为 Departure Mono 字形转路径，无字体依赖）。组件内联版本为 `PixelMark`（门缝以 evenodd 镂空实现，透出底层 accent 色），必须与 SVG 资产保持同一图形。
