...
AgentAdapter         Codex / Pi / Cursor / Claude Code / custom
CapabilityAdapter    Skill / MCP / CLI / HTTP
HostAdapter          macOS / Windows / Linux
StoreAdapter         SQLite / PostgreSQL（未来）
```

增加一个 Agent、渠道或外部系统时，优先增加 Adapter 和 Profile，不修改 Session、Flow 和 Run 生命周期。
...
- Agent 后端更换不影响历史 Session 的 ContextSnapshot、Run 和 Evidence；Flow 的定义版本独立于 Agent。
