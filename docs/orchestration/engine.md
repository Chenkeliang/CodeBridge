...
执行后必须写入成功或失败事件，并保存必要的输出 Artifact。Capability Adapter 返回的内容、diff、日志和测试报告以 `artifact://<id>` 引用保存；结构化验证结果写入 Verification，并与 Artifact ID 关联。列表接口默认只返回元数据，内容需要显式读取，避免事件流膨胀。外部系统超时会进入失败或结果未知状态。
