...
公司本来就用飞书，所以搞了个小东西 **CodeBridge**：飞书 @ 机器人发句话，家里/办公室那台 Mac 上的 Runner 拉起 `cursor-agent` / `claude` / `codex`，结果流式回飞书。
...
- `appId` / `appSecret` 写入 `~/.codebridge/config.yaml`

飞书是遥控器和聊天窗口，真正干活的是本机 CLI；也可以只配置 Telegram Bot 使用。
...
- 同一飞书会话连发消息，后续任务会排队；`/stop` 可中断并清空队列
...
- 开源 MIT：[CodeBridge](https://github.com/Chenkeliang/CodeBridge)
