...
重复卡按同一来源、业务对象及问题归并；仅展示用的独立告警时间/次数、trace/request 行不作为区分依据，保留单号、SKU、仓库、数量及错误条件。未结案件保持关联，本人结案后同案重复卡继承 DONE；不同业务对象不因标题相同而合并。直接点击 DONE 是否作为本人确认输入、同类不同单号是否批量结案，仍待用户选择；当前实现明确的话题文字入口。
