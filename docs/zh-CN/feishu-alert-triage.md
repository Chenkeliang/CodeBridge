...
优先 SKILL、矩阵与同群同类型的历史记录；已明确的通知类按规则直接归类，只有需要本单状态时才补最小查询。拿不准先 @ 负责人，不要求每条都查代码、流程和日志。

系统依据规则判定无需操作仍为 `no_action` / CrossMark。本人在关联话题明确回复“无需处理”“不用处理”“不需要处理”等，后端验证真实发送人后记录 `dismissed` / DONE，停止本案排查；这表示本人结案，不表示修复成功。Agent 状态命令不能伪造本人结案，迟到结果也不能覆盖。本人明确“重新排查”可重新开启。

排队也先标 OnIt；正文不可读的每张卡单独进入 waiting 并请求本人补充，不能因缺内容或去重而不登记状态。投递失败仍有持久待投递记录，不能声称接口失败时表情已经可见。

重复卡按同一来源、业务对象及问题归并；仅展示用的独立告警时间/次数、trace/request 行不作为区分依据，保留单号、SKU、仓库、数量及错误条件。未结案件保持关联，本人结案后同案重复卡继承 DONE；不同业务对象不因标题相同而合并。本人直接在任一关联原卡片点击飞书 DONE 即结案，同案重复卡同步 DONE；不再另外询问原因。其他人的 DONE、小 V 自己的 DONE 和删除表情均不触发结案/重开。SDK reaction 事件及时处理，定时只读回查每群每轮最多 20 张已登记卡片，持久化轮转进度补偿断线；积压时完成回查可能超过一分钟。明确重新排查后，不将旧 DONE 当作再次结案。

普通告警逐个确认原因，不同订单/任务不能只按标题批量关闭。纯通知只有匹配 SKILL 同目录 `references/notification-rules.json` 内唯一已核实规则才一起结束：规则需 verified=true、kind=notification、明确来源、完整标题、逐行原因条件和证据。每张原卡片分别标记 CrossMark；本人再点 DONE 则记录本人结案。缺规则、草稿或匹配歧义均回退逐条判断；当前通知示例规则仍是未启用草稿。
