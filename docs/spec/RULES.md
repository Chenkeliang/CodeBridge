...
| FE-I18N-001 | SHOULD | review | 界面文案统一中文;技术名词(Session/Run/Agent)保留英文 |
...
| ARCH-002 | MUST | review | Control Plane 各 Catalog(Agent/Session/Capability)只经事件互联,不直连成网 |
...
| PROTO-PROMPT-002 | MUST | review | 可变内容只追加不重排;厚重不变内容(能力 manifest)走工具懒加载,不内联进提示词 |
...
| SEC-002 | MUST | review | 生产写默认审批;审批 token 绑定 当前 Run、操作目标与参数 hash |
