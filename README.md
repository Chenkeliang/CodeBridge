...
The monorepo contains `core`, `backends`, `work-items`, `workflow-engine`, `policy`, `run-executor`, `runner-host`, `runner-client`, `router`, `channel-feishu`, `channel-telegram`, and `apps/bridge`.
